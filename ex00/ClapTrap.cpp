...
void ClapTrap::attack(ClapTrap &target) {
    ...
    else if (!_attackDamage)
    {
        ...
        std::cout << "ClapTrap " << _name << " tried to attack but has no power!" << std::endl;
...
void ClapTrap::beRepaired( unsigned int amount )
{
    ...
    if (_energyPoints >= 0)
    {
        _energyPoints--;
        _hitPoints += amount;
        std::cout << "ClapTrap " << _name << " is repaired by " << amount << " points and now has " << _hitPoints << " hitpoints!" << std::endl;
    }
    else
...
void ClapTrap::setAttackDamage( unsigned int amount )
{
    ...
    std::cout << "ClapTrap " << _name << " is preparing their fists. Their attack damage is at " << _attackDamage << std::endl;
}
